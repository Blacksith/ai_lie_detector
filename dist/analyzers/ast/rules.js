...
function nameAppearsOutsideImport(content, name, importLine) {
    const lines = content.split(/\r?\n/);
    const safeName = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const re = new RegExp(`\\b${safeName}\\b`);
    for (let i = 0; i < lines.length; i += 1) {
        if (importLine && i + 1 === importLine)
            continue;
        if (re.test(lines[i]))
            return true;
    }
...
export const unusedImportsRule = {
    ...
    run(ast, ctx) {
        ...
        traverse(ast, {
            ImportSpecifier(path) {
                ...
                if (path.node.importKind === 'type' || parentImportKind === 'type')
                    return;
...
export const unreachableCodeRule = {
...
export const suspiciousAsyncRule = {
...
export const fakeImportSymbolRule = {
...
export const consoleInCodeRule = {
    id: 'ast.console_left',
...
export const todoDensityRule = {
    id: 'ast.todo_density',
    title: 'High TODO/FIXME density',
    defaultSeverity: 'info',
    run(_ast, ctx) {
        const matches = ctx.file.content
            .split(/\r?\n/)
