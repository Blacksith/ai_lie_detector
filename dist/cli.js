...
program
    ...
    .action(async (repoPath, flags) => {
    const onlyExt = typeof flags.only === 'string' ? flags.only.split(',').map((x) => x.trim()).filter(Boolean) : undefined;
    const extraExclude = typeof flags.exclude === 'string'
        ? flags.exclude.split(',').map((x) => x.trim()).filter(Boolean)
        : undefined;
    const outputFormat = flags.json || flags.format === 'json' ? 'json' : 'text';
